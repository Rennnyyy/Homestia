...
When the user asks for a skill, invokes a spirit, or says `/aletheia`, `/imagine`, `/scaffold`, `/craft`, `/refactor`, `/audit`, `/ship`, `/seal`, or any related command — **do not guess**. Read the corresponding SKILL.md from the Aletheia workspace first.
