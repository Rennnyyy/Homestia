...
- **Engine:** `ShaclValidatorService` in `src/app/core/shapes/` fetches shapes, parses them, converts form JSON into RDF (predicate namespace = `https://homestia.katharsis.digital/predicates/homestia/`, local name = JSON key), and maps violation reports back to JSON paths (`rooms[0].roomSize`).
